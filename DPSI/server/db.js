...
function queueSave() {
  if (_saveScheduled) { _savePending = true; return }
  _saveScheduled = true
  _savePending = false
  setImmediate(() => {
    _saveScheduled = false
    saveDb()
    if (_savePending) queueSave()
  })
}
...
function migrate() {
  try { db.execRaw("ALTER TABLE penempatan_guru ADD COLUMN hari TEXT DEFAULT NULL") } catch (e) {}
  try { db.execRaw("ALTER TABLE penempatan_guru ADD COLUMN jamMulai TEXT DEFAULT NULL") } catch (e) {}
  try { db.execRaw("ALTER TABLE penempatan_guru ADD COLUMN jamSelesai TEXT DEFAULT NULL") } catch (e) {}
}
...
function seed() {
  ...
  try {
    for (const u of [
      ...
      db.runRaw('INSERT INTO users (id, username, password, name, role, mapel, waliKelas) VALUES (?, ?, ?, ?, ?, ?, ?)', u)
    }

    for (const k of ['7A','7B','8A','8B','9A','9B']) {
      db.runRaw('INSERT INTO kelas (id, tingkat, nama) VALUES (?, ?, ?)', [k, k[0], k])
    }

    const mapels = ['Matematika','Bahasa Indonesia','Bahasa Inggris','IPA','IPS','Pendidikan Agama Islam','PKN']
    mapels.forEach((m, i) => db.runRaw('INSERT INTO mapel (id, nama) VALUES (?, ?)', [i + 1, m]))
    ...
    for (const s of siswaData) {
      db.runRaw('INSERT INTO siswa (id, nis, nama, kelasId) VALUES (?, ?, ?, ?)', s)
    }

    const tahun = getTahunAjaran()
    db.runRaw('INSERT INTO penempatan_guru (guruId, mapelId, kelasId, tahunAjaran, hari, jamMulai, jamSelesai) VALUES (?, ?, ?, ?, ?, ?, ?)', [1, 1, '7A', tahun, 'Senin', '07:00', '08:30'])
    db.runRaw('INSERT INTO penempatan_guru (guruId, mapelId, kelasId, tahunAjaran, hari, jamMulai, jamSelesai) VALUES (?, ?, ?, ?, ?, ?, ?)', [1, 1, '7B', tahun, 'Rabu', '08:40', '10:10'])
